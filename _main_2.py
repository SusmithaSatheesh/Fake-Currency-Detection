...
root ="F:\Python Project\Python_Fake Currency Detection/Image/new_m/"
   ...
   # sample = getFeatureImage(proper_image, feature_set[4])
    ...
    for x_out, d_feat in enumerate(detect_feat):
        ...
        for i_note in range(0, len(notes_type)-1):
           ...
           # reference_image = cv2.imread('../../../Image/' + notes_type[i_note]+'/000' + str(d_feat+1) + '.jpg', 0)
            reference_image = cv2.imread('F:\Python Project\Python_Fake Currency Detection/Image/' + notes_type[i_note]+'/000' + str(d_feat+1) + '.jpg', 0)
            try:
    ...
    # =======================  VERIFICATION  OF NEW NOTES===========================================
        ...
        for iter, feature_x in enumerate(verify_feat):
            ...
            reference_image = cv2.imread('F:\Python Project\Python_Fake Currency Detection/Image/' + notes_type[lead]+'/000' + str(feature_x+1) + '.jpg', 0)
    ...
    # ======================= FOR OLD NOTES ONLY WHEN UNDETECTED ======================
        ...
        for x_out, d_feat in enumerate(detect_feat_old):
            ...
            for i_note in range(0, len(notes_type_old)-1):
                ...
                reference_image = cv2.imread('F:\Python Project\Python_Fake Currency Detection/Image/' + notes_type_old[i_note]+'/000' + str(d_feat+1) + '.jpg', 0)
                try:
                    matches , kp = imageMatcher(reference_image, sample)
                    if matches == None or kp == None:
...
# ============================= OLD NOTES VERIFICATION ==========================================
        if lead !=4:
            ...
            for iter, feature_x in enumerate(verify_feat_old):
                ...
                reference_image = cv2.imread('F:\Python Project\Python_Fake Currency Detection/Image/' + notes_type_old[lead]+'/000' + str(feature_x+1) + '.jpg', 0)
                try:
                    matches , kp = imageMatcher(reference_image, sample)
                    if matches == None or kp == None:
